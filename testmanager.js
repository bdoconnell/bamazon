...
function logItems(result) {
    ...
    for (var i = 0; i < res.length; i++) {
        table.push([itemID, product_name, department_name, item_cost, stock_quantity]);
    }
    ...
    function addInventory() {
        return inquirer.prompt([{
            ...
            return new Promise(function (resolve, reject) {
                connection.query('SELECT stock_quantity FROM products WHERE ?', { itemID: answer.item }, function (err, res) {
                    if (err) reject(err);
                    resolve(res);
                });
            }).then(function (result) {
